      ...
      try {
        await fetch("/api/send-rejection", {
          ...
          body: JSON.stringify({
            ...
            date: format(new Date(rejectingReservation.date), "EEEE, MMMM d, yyyy", {
              locale: dateLocale,
            }),
            time: rejectingReservation.time.slice(0, 5),
    ...
    <div className="min-h-screen bg-slate-900 dark:bg-[#0c0a1d] overflow-x-hidden pt-20 md:pt-24">
      ...
      {rejectionModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          {/* Modal */}
          <div className="relative bg-white dark:bg-[#13102b] rounded-2xl shadow-2xl w-full max-w-lg border border-slate-200 dark:border-[#2d2a4a]">
            {/* Header */}
            ...
            <div className="p-6">
              ...
              {rejectingReservation && (
                <div className="mb-4 p-4 bg-slate-50 dark:bg-[#1a1735] rounded-xl">
                  ...
                  <div className="flex flex-wrap gap-4 text-sm text-slate-600 dark:text-slate-400">
                    <span className="flex items-center gap-1">
                      ...
                      {format(new Date(rejectingReservation.date), "MMM d, yyyy", { locale: dateLocale })}
                    </span>
                    <span className="flex items-center gap-1">
                      <Clock size={14} className="text-amber-500" />
                      {rejectingReservation.time.slice(0, 5)}
                    </span>
                  </div>
                </div>
              )}
                      ...
                      <div className="space-y-2">
                        ...
                        {(reservation.budget || reservation.property_type || reservation.investment_type || reservation.reason) && (
                          <div className="mt-3 pt-3 border-t border-slate-100 dark:border-[#2d2a4a]">
                            <div className="flex flex-wrap gap-3 text-sm">
                              {reservation.budget && (
                                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400 rounded-md">
                                  <DollarSign size={12} />
                                  {reservation.budget}
                                </span>
                              )}
                              {reservation.property_type && (
                                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-blue-50 dark:bg-blue-900/20 text-blue-700 dark:text-blue-400 rounded-md capitalize">
                                  <Home size={12} />
                                  {reservation.property_type}
                                </span>
                              )}
                              {reservation.investment_type && (
                                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-purple-50 dark:bg-purple-900/20 text-purple-700 dark:text-purple-400 rounded-md capitalize">
                                  <Key size={12} />
                                  {reservation.investment_type}
                                </span>
                              )}
                              {reservation.reason && (
                                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-orange-50 dark:bg-orange-900/20 text-orange-700 dark:text-orange-400 rounded-md capitalize">
                                  <Target size={12} />
                                  {reservation.reason}
                                </span>
                              )}
                        ...
                        {reservation.referral_source && (
                          <div className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-500 mt-2">
                            <Globe size={12} />
                            <span>Found via: <span className="capitalize">{reservation.referral_source}</span></span>
                          </div>
                        )}
                        ...
                        {reservation.message && (
                          <div className="flex items-start gap-1 text-sm text-slate-600 dark:text-slate-400 mt-3 pt-3 border-t border-slate-100 dark:border-[#2d2a4a]">
                            <MessageSquare
                              size={14}
                              className="mt-0.5 shrink-0"
                            />
                            <span className="italic">&quot;{reservation.message}&quot;</span>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="flex gap-2">
                        {reservation.status === "pending" && (
                          <>
                            ...
                            <button
                              onClick={() => openRejectionModal(reservation)}
